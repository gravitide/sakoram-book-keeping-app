<template>
	<UModal v-model:open="openModel" :title="`Generate ${pending.length} pending ${pending.length === 1 ? 'invoice' : 'invoices'}`">
		<template #body>
			<div class="space-y-4">
				...
				<div v-if="pending.length === 0" class="py-6 text-center text-sm text-(--ui-text-muted)">
					<UIcon name="i-lucide-check-circle-2" class="size-8 mx-auto mb-2 opacity-60" />
